...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 28-05-2019 a las 20:43:53
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `PA_Saldo_Usuario` (IN `pID` INT, OUT `oNombre` VARCHAR(100), OUT `oApellido` VARCHAR(100), OUT `oSaldo` DOUBLE)  BEGIN
	select usu.nombre,usu.apellido,misa.saldo
	INTO oNombre,oApellido,oSaldo
    from mi_saldo misa, usuarios usu, cuentas cu
    WHERE cu.id_Cuenta = misa.id_Cuenta
    AND cu.id_Usuario = usu.id 
    AND usu.id = pID;
END$$
...
INSERT INTO `movimientoscuentas` (`id`, `tipo_movimiento`, `id_Cuenta`, `monto`, `Saldo_Anterior`, `Saldo_Nuevo`, `fechaTransaccion`) VALUES
(1, 'Transaccion', 4, 0, 0, 3000, '2019-05-28 18:11:38'),
(2, 'Retiro', 4, 0, 3000, 2450, '2019-05-28 18:11:38'),
(3, 'Deposito', 4, 200, 2450, 2650, '2019-05-28 18:11:38'),
(4, 'Retiro', 4, -350, 2650, 2300, '2019-05-28 18:11:38'),
(5, 'Retiro', 4, -500, 2300, 1800, '2019-05-28 18:11:38'),
(6, 'Retiro', 4, -300, 1800, 1500, '2019-05-28 18:11:38'),
(7, 'Retiro', 4, -250, 1500, 1250, '2019-05-28 18:11:38'),
(8, 'Retiro', 4, -150, 1250, 1100, '2019-05-28 18:11:38'),
(9, 'Retiro', 4, -300, 1100, 800, '2019-05-28 18:12:36'),
(10, 'Retiro', 4, -200, 800, 600, '2019-05-28 18:14:05');
